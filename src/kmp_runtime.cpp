...
#if (KMP_ARCH_X86_64 || KMP_ARCH_ARM || KMP_ARCH_AARCH64) && (KMP_OS_LINUX || KMP_OS_VXWORKS)
                    va_list *ap
#else
                    va_list ap
#endif
...
static inline char *__kmp_reg_status_name() {
  ...
  return __kmp_str_format("__KMP_REGISTERED_LIB_%ld", (long)getpid());
...
#if !KMP_DYNAMIC_LIB && !KMP_OS_VXWORKS
  {
...
#if KMP_OS_LINUX || KMP_OS_WINDOWS || KMP_OS_HURD || KMP_OS_VXWORKS
