...
void __kmp_create_worker(int gtid, kmp_info_t *th, size_t stack_size) {
  ...
  {
      char worker_name[20];
      // Name the works "tOpenMPx" in VxWorks
      snprintf(worker_name, sizeof worker_name, "tOpenMP%d", gtid);
      pthread_attr_setname(&thread_attr, worker_name);
  }
#endif /* KMP_OS_VXWORKS */
...
void __kmp_register_atfork(void) {
  if (__kmp_need_register_atfork) {
    ...
    int status = pthread_atfork(__kmp_atfork_prepare, __kmp_atfork_parent,
                                __kmp_atfork_child);
#else
    // Fork will never happen as it does not exist in VxWorks
    int status = 0;
#endif
...
#elif KMP_OS_VXWORKS
  cpuset_t availCpus = vxCpuEnabledGet ();
  cpuset_t ix;

  for (ix = 0; ix < 32; ++ix) {
      if (CPUSET_ISSET (availCpus, ix)) {
          ++r;
      }
  }
#else
...
#if KMP_OS_VXWORKS
#ifdef _WRS_KERNEL
static int vxworks_is_init = 0;
extern "C" void ompInit() {
    // To detect if OpenMP is used but INCLUDE_OPENMP is not added to the VIP,
    // see __kmp_runtime_initialize() below.
    vxworks_is_init = 1;
}
#else /* RTP */
static int vxworks_is_init = 1;
#endif /* _WRS_KERNEL */
#endif /* KMP_OS_VXWORKS */
...
#if KMP_OS_VXWORKS
  if (!vxworks_is_init) {
      KMP_FATAL(MsgExiting, "Must add component INCLUDE_OPENMP to use this feature");
  }
...
#elif KMP_OS_DRAGONFLY || KMP_OS_OPENBSD || KMP_OS_VXWORKS
